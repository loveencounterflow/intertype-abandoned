...
(function() {
  'use strict';
  var CND, _xrpr, alert, assign, badge, cnd_by_ity, debug, flatten, get_rprs_of_tprs, help, info, inspect, isa, isa_type, ity_by_cnd, jr, rpr, tester_from_tests_object, urge, validator_from_tests_object, warn, whisper, xrpr,
    indexOf = [].indexOf,
  ...
  // ADDING TYPES
  //-----------------------------------------------------------------------------------------------------------
  tester_from_tests_object = function(type, tests) {
    return function(x) {
      var name, test;
...
/* TAINT use `name` to improve error messages */
  ...
  //-----------------------------------------------------------------------------------------------------------
  validator_from_tests_object = function(type, tests) {
    return function(x) {
      var name, test;
      for (name in tests) {
        test = tests[name];
  ...
  this.add_type = function(type, settings, tester) {
    var _type, arity, defaults, ref, ref1, supertype;
    ...
    //.........................................................................................................
    /* Add supertype: */
    if ((supertype = settings.supertype) != null) {
      this.add_supertype(type, supertype);
    }
    //.........................................................................................................
    /* Add type tester method: */
    this[type] = (x, ...tprs) => {
      return tester(x, ...tprs);
    };
  ...
  //-----------------------------------------------------------------------------------------------------------
  this.add_supertype = function(subtype, supertype) {
    var arity, type;
  ...
  this.extends = function(subtype, supertype) {
    ...
    return this._extends(subtype, supertype, [subtype]);
  };

  //-----------------------------------------------------------------------------------------------------------
  this._extends = function(subtype, supertype, trail) {
    var subsupertype;
    if (subtype === supertype) {
      return true;
    }
    if ((subsupertype = this.supertypes[subtype]) == null) {
      return false;
    }
    if (indexOf.call(trail, subsupertype) >= 0) {
      throw new Error(`µ44857 detected loop in supertypes: ${rpr(trail)}`);
    }
    trail.push(subsupertype);
    if (subsupertype === supertype) {
      return true;
    }
    return this._extends(subsupertype, supertype, trail);
  };
  ...
  this.supertype_of_type = function(type) {
    var supertype;
    if ((supertype = this.supertypes[type]) == null) {
      return type;
    }
...
//---------------------------------------------------------------------------------------------------------
  ...
  //-----------------------------------------------------------------------------------------------------------
  isa.add_type('integer', {
    supertype: 'number'
  }, Number.isInteger);

  isa.add_type('finite_number', {
    supertype: 'number'
  }, Number.isFinite);

  isa.add_type('positive', {
    supertype: 'number'
  }, function(x) {
    return (this.number(x)) && (x > 0);
  });

  isa.add_type('negative', {
    supertype: 'number'
  }, function(x) {
    return (this.number(x)) && (x < 0);
  });

  isa.add_type('nonnegative', {
    supertype: 'number'
  }, function(x) {
    return (this.number(x)) && (x >= 0);
  });

  isa.add_type('multiple_of', {
    supertype: 'number'
  }, function(x, d) {
  ...
  //-----------------------------------------------------------------------------------------------------------
  isa.add_type('asyncfunction', {
    supertype: 'callable'
  }, function(x) {
    return (this.type_of(x)) === 'asyncfunction';
  });

  isa.add_type('boundfunction', {
    supertype: 'callable'
  }, function(x) {
